...
describe("onboarding tour steps", () => {
	it("builds contribute steps with expected selectors and copy", () => {
		...
		expect(steps[1].element).toBe("[data-tour='contribute-location']");
		expect(steps[2].element).toBe("[data-tour='contribute-qr-upload']");
		expect(steps[3].element).toBe("[data-tour='contribute-submit']");
		expect(steps[0].popover?.title).toBe("Menu navigasi");
		expect(steps[1].popover?.title).toBe("Lokasi semasa");
	});
	...
	it("uses fallback element resolver for rejected-step in my-contributions", () => {
		...
		expect(step.popover?.description).toContain("Edit semula");
	...
	it("returns fallback selector only for tuple-based steps", () => {
		...
		expect(getFallbackSelector("/contribute", 0)).toBe(
