...
type Props = {
	...
	onChange: (props: { categories: string[]; state: string }) => void;
	institutions: Institution[];
	className?: string;
};

const Filters = forwardRef<HTMLDivElement, Props>(
	({ onSearch, onChange, institutions, className }) => {
				...
				<div className="flex flex-col items-center justify-center gap-2 sm:flex-row">
					...
					<div className="w-full">
						<Input
							...
							className={cn(
								"w-full rounded-lg bg-background text-sm border",
								className,
							)}
							value={searchQuery}
