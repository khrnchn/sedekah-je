...
async function verifyMigration() {
	...
	try {
		// Check total count
		const totalCount = await db.select({ count: sql<number>`count(*)` }).from(institutions);
		...
		}).from(institutions).groupBy(institutions.category);
		...
		const byState = await db.select({
			...
			count: sql<number>`count(*)`
		}).from(institutions).groupBy(institutions.state).orderBy(sql`count(*) desc`).limit(10);
		...
		samples.forEach((institution, index) => {
			console.log(`  ${index + 1}. ${institution.name} (${institution.category}) - ${institution.city}, ${institution.state}`);
		});
		
		// Check for required fields
		const missingRequired = await db.select({
			id: institutions.id,
